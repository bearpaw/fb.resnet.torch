...
local function createModel(opt)
   ...
   local function bottleneck(n, stride, type)
      ...
      -- Pyramid
      local C = 4     
      local D = math.min(math.floor(nInputPlane / C), 32)
      ...
      if inputRes >= 7 then 
         ...
         if type ~= 'no_preact' then
            pyra:add(SBatchNorm(nInputPlane))
            pyra:add(ReLU(true))
         end
         pyra:add(Convolution(nInputPlane, D,1,1,stride,stride))
         pyra:add(SBatchNorm(D))
         pyra:add(ReLU(true))
         pyra:add(pyramid(D, C))
         pyra:add(SBatchNorm(D))
         pyra:add(ReLU(true))
         pyra:add(Convolution(D, n*4,1,1,stride,stride)) 
